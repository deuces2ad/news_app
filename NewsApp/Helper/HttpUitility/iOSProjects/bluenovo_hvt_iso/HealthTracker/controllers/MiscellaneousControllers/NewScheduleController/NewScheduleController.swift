...
class NewScheduleController: InterfaceExtendedController {
    override func viewDidLoad() {
        super.viewDidLoad()
        setupView()
        view.backgroundColor = .white
        paintLogout()
        paintNavigationTitle(text: "New Appointment".localized())
        paintHideBackButton()
    }
    
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(true)
        tabBarController?.title = "New Appointment".localized()
    }
    
    lazy var lbl : UILabel = {
        let lbl = UILabel()
        lbl.text = "Under Development".localized()
