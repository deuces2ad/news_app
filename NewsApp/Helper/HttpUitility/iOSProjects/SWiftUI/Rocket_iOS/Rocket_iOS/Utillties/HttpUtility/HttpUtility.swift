...
public struct HttpUtility
{
    ...
    private func getData<T:Decodable>(requestUrl: URL, resultType: T.Type, completionHandler:@escaping(Result<T?, HUNetworkError>)-> Void)
    {
        ...
        Log.echo(key: "HttpUtility", text: " urlRequest: \(urlRequest)")
        ...
        URLSession.shared.dataTask(with: requestUrl) { (data, httpUrlResponse, error) in
            ...
            if(error == nil && data != nil && data?.count != 0) {
                ...
                if(response != nil) {
                    ...
                    completionHandler(.failure(HUNetworkError(withServerResponse: data, forRequestUrl: requestUrl.url!, withHttpBody: requestUrl.httpBody, errorMessage: error.debugDescription, forStatusCode: statusCode ?? 404)))
                }
            }
            else {
                let networkError = HUNetworkError(withServerResponse: data, forRequestUrl: requestUrl.url!, withHttpBody: requestUrl.httpBody, errorMessage: error.debugDescription, forStatusCode: statusCode ?? 404)
